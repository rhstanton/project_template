...
class TestNotebookEnvironment:
    ...
    def test_runnotebook_unsets_cdpath(self, runnotebook_wrapper):
        ...
        with open(runnotebook_wrapper, 'r') as f:
            content = f.read()
...
class TestNotebookExecution:
    ...
    def test_parameter_injection(self, sample_notebook, tmp_path, repo_root):
        ...
        run_command([
            ...
            str(sample_notebook),
            str(output_nb),
            "-p", "study", "injected_value",
            "-p", "out_file", "custom_output.txt"
        ], cwd=repo_root)
        ...
        for cell in nb.cells:
            if cell.cell_type == 'code' and 'injected-parameters' in cell.metadata.get('tags', []):
                assert 'injected_value' in cell.source, "Parameter not injected"
                assert 'custom_output.txt' in cell.source, "Custom parameter not injected"
                break
        else:
...
class TestNotebookOutputs:
    ...
    def test_executed_notebook_saved(self, repo_root):
        ...
        exec_nb_path = repo_root / "output" / "executed_notebooks" / "correlation_analysis_executed.ipynb"
...
class TestMakefileIntegration:
    ...
    def test_make_correlation_succeeds(self, repo_root):
        ...
        success = (result.returncode == 0 or
                  "Nothing to be done" in result.stdout or
...
class TestNotebookErrorHandling:
    ...
    def test_missing_parameters_cell_fails(self, tmp_path, repo_root):
        ...
        assert result.returncode != 0 or "warning" in result.stderr.lower() or \
               "parameter" in result.stderr.lower(), \
