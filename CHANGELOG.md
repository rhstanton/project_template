...
- **Bumped the Python requirement to 3.12** (`requires-python = ">=3.12,<3.13"`; ruff `target-version`/mypy `python_version` updated to `3.12`). *Why:* uv's CPython for 3.12 links OpenSSL 3.5 on macOS as well as Linux, so juliapkg installs **Julia 1.12 on every platform** — eliminating the earlier macOS-1.11-vs-Linux-1.12 split and making local builds run the identical Julia version as the Docker artifact.
- **Julia bridge updates:** bumped `juliacall`/`PythonCall` to `0.9.34`; pinned `pandas<3` (PythonCall cannot convert pandas-3 DataFrames to Julia); removed unused `Arrow`/`RDatasets` from `env/Project.toml`. The Julia version is derived from the Python interpreter's OpenSSL — on Python 3.12 that is OpenSSL 3.5 on both macOS and Linux/Docker, so Julia resolves to **1.12 everywhere** — see [docs/julia_python_integration.md](docs/julia_python_integration.md) "Version coupling & gotchas".
