...
def run_julia_did(df: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    # Build formula
    interaction_terms = [
        f"_time_{int(t) if t >= 0 else f'm{abs(int(t))}'}" for t in times
    ]
...
def run_pyfixest_did(df: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    coef_df = pd.DataFrame(
        {
            "relative_year": [
                int(c.split("[T.")[1].split("]")[0]) for c in event_coefs
            ],
...
def plot_event_study(coef_df: pd.DataFrame, config: dict) -> None:
    ...
    ax.set_title(
        "Difference-in-Differences Event Study", fontsize=14, fontweight="bold"
    )
...
def main() -> None:
    """Main entry point"""
    args = friendly_docopt(__doc__, version="2.0.1")
    setup_environment()
    ...
    print(
        f"Backend: {'Julia (FixedEffectModels)' if config['use_julia'] else 'Python (pyfixest)'}"
    )
    print("=" * 70 + "\n")
